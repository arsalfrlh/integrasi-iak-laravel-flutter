...
class BeliIakPage extends StatefulWidget {
  BeliIakPage({required this.pricelist});
  Pricelist pricelist;

  @override
  State<BeliIakPage> createState() => _BeliIakPageState();
}

class _BeliIakPageState extends State<BeliIakPage> {
  final ApiService apiService = ApiService();
  final kostumerID = TextEditingController();
  late TextEditingController nmProdukController;
  late TextEditingController detailProdukController;
  late TextEditingController hargaController;

  @override
  void initState() {
    super.initState();
    nmProdukController = TextEditingController(text: widget.pricelist.namaProduk);
    detailProdukController = TextEditingController(text: widget.pricelist.detailProduk);
    hargaController = TextEditingController(text: widget.pricelist.harga.toString());
  }

  void _beli(BuildContext context)async{
    if(widget.pricelist.codeProduk.isNotEmpty && kostumerID.text.isNotEmpty){
      showDialog(
        context: context, 
        barrierDismissible: false,
        builder: (context) => const Center(child: CircularProgressIndicator(),));

      final response = await apiService.beliProdukIAK(kostumerID.text, widget.pricelist.codeProduk);
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  SignUpForm(
                    nmProduk: nmProdukController,
                    deskripsi: detailProdukController,
                    harga: hargaController,
                    kostumerID: kostumerID,
                    beli: () => _beli(context),
                  ),
                ],
              ),
...
class SignUpForm extends StatelessWidget {
  const SignUpForm({required this.nmProduk, required this.deskripsi, required this.harga, required this.kostumerID, required this.beli});
  final TextEditingController nmProduk, deskripsi, harga, kostumerID;
  final VoidCallback beli;
    ...
    return Form(
      child: Column(
        children: [
          ...
          SizedBox(height: 24,),
          TextFormField(
            controller: kostumerID,
            decoration: InputDecoration(
                labelText: "Kostumer ID",
                hintText: "No: 08xxx/ ID: 123xx",
                floatingLabelBehavior: FloatingLabelBehavior.always,
                hintStyle: const TextStyle(color: Color(0xFF757575)),
                contentPadding: const EdgeInsets.symmetric(
                  horizontal: 24,
